...
int main(void) {
    // F0 uses T_00, T_01, T_02 and T_03
    unsigned char T_00[256][4];
    for(int i = 0; i < 256; ++i) {
        T_00[i][0] = clefia_s0[i];
        T_00[i][1] = ClefiaMul2(clefia_s0[i]);
        T_00[i][2] = ClefiaMul4(clefia_s0[i]);
        T_00[i][3] = ClefiaMul6(clefia_s0[i]);
    }
    printf("const unsigned long T_00[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_00[i * 8 + j][0], T_00[i * 8 + j][1], T_00[i * 8 + j][2], T_00[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    unsigned char T_01[256][4];
    for(int i = 0; i < 256; ++i) {
        T_01[i][0] = ClefiaMul2(clefia_s1[i]);
        T_01[i][1] = clefia_s1[i];
        T_01[i][2] = ClefiaMul6(clefia_s1[i]);
        T_01[i][3] = ClefiaMul4(clefia_s1[i]);
    }
    printf("const unsigned long T_01[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_01[i * 8 + j][0], T_01[i * 8 + j][1], T_01[i * 8 + j][2], T_01[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    unsigned char T_02[256][4];
    for(int i = 0; i < 256; ++i) {
        T_02[i][0] = ClefiaMul4(clefia_s0[i]);
        T_02[i][1] = ClefiaMul6(clefia_s0[i]);
        T_02[i][2] = clefia_s0[i];
        T_02[i][3] = ClefiaMul2(clefia_s0[i]);
    }
    printf("const unsigned long T_02[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_02[i * 8 + j][0], T_02[i * 8 + j][1], T_02[i * 8 + j][2], T_02[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    unsigned char T_03[256][4];
    for(int i = 0; i < 256; ++i) {
        T_03[i][0] = ClefiaMul6(clefia_s1[i]);
        T_03[i][1] = ClefiaMul4(clefia_s1[i]);
        T_03[i][2] = ClefiaMul2(clefia_s1[i]);
        T_03[i][3] = clefia_s1[i];
    }
    printf("const unsigned long T_03[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_03[i * 8 + j][0], T_03[i * 8 + j][1], T_03[i * 8 + j][2], T_03[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    // F1 uses T_10, T_11, T_12 and T_13
    unsigned char T_10[256][4];
    for(int i = 0; i < 256; ++i) {
        T_10[i][0] = clefia_s1[i];
        T_10[i][1] = ClefiaMul8(clefia_s1[i]);
        T_10[i][2] = ClefiaMul2(clefia_s1[i]);
        T_10[i][3] = ClefiaMulA(clefia_s1[i]);
    }
    printf("const unsigned long T_10[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_10[i * 8 + j][0], T_10[i * 8 + j][1], T_10[i * 8 + j][2], T_10[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    unsigned char T_11[256][4];
    for(int i = 0; i < 256; ++i) {
        T_11[i][0] = ClefiaMul8(clefia_s0[i]);
        T_11[i][1] = clefia_s0[i];
        T_11[i][2] = ClefiaMulA(clefia_s0[i]);
        T_11[i][3] = ClefiaMul2(clefia_s0[i]);
    }
    printf("const unsigned long T_11[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_11[i * 8 + j][0], T_11[i * 8 + j][1], T_11[i * 8 + j][2], T_11[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    unsigned char T_12[256][4];
    for(int i = 0; i < 256; ++i) {
        T_12[i][0] = ClefiaMul2(clefia_s1[i]);
        T_12[i][1] = ClefiaMulA(clefia_s1[i]);
        T_12[i][2] = clefia_s1[i];
        T_12[i][3] = ClefiaMul8(clefia_s1[i]);
    }
    printf("const unsigned long T_12[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_12[i * 8 + j][0], T_12[i * 8 + j][1], T_12[i * 8 + j][2], T_12[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");

    unsigned char T_13[256][4];
    for(int i = 0; i < 256; ++i) {
        T_13[i][0] = ClefiaMulA(clefia_s0[i]);
        T_13[i][1] = ClefiaMul2(clefia_s0[i]);
        T_13[i][2] = ClefiaMul8(clefia_s0[i]);
        T_13[i][3] = clefia_s0[i];
    }
    printf("const unsigned long T_13[256] = {\n");
    for(int i = 0; i < 32; ++i) {
        printf(" ");
        for(int j = 0; j < 8; ++j) {
            printf(" 0x%02x%02x%02x%02x,", T_13[i * 8 + j][0], T_13[i * 8 + j][1], T_13[i * 8 + j][2], T_13[i * 8 + j][3]);
        }
        printf("\n");
    }
    printf("};\n\n");
