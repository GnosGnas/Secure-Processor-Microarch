...
void ClefiaF0Xor(unsigned char *dst, const unsigned char *src, const unsigned char *rk)
{
  ...
  /* Newly added T-table look-up code */
  unsigned long value = T_00[x[0]] ^ T_01[x[1]] ^ T_02[x[2]] ^ T_03[x[3]];
  y[0] = (value >> 24) & 0xFF;
  y[1] = (value >> 16) & 0xFF;
  y[2] = (value >>  8) & 0xFF;
  y[3] = (value >>  0) & 0xFF;
  /* Above 4 lines will place the required values in array y as expected by the next lines of code */
...
void ClefiaF1Xor(unsigned char *dst, const unsigned char *src, const unsigned char *rk)
{
  ...
  /* Newly added T-table look-up code */
  unsigned long value = T_10[x[0]] ^ T_11[x[1]] ^ T_12[x[2]] ^ T_13[x[3]];
  y[0] = (value >> 24) & 0xFF;
  y[1] = (value >> 16) & 0xFF;
  y[2] = (value >>  8) & 0xFF;
  y[3] = (value >>  0) & 0xFF;
